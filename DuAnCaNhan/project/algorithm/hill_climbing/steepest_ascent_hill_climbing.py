...
def steepest_ascent_hill_climbing(start, dirties, cols, rows):
    ...
    yield {
        ...
        "frontier": [],
        "explored": explored,
        "log": f"Steepest-Ascent Hill Climbing khởi tạo tại {start}, rác còn: {len(dirties)}"
    }
    ...
    while True:
        ...
        if not children:
            yield {
                ...
                "done": True,
                "path": reconstruct_path(current_node),
                "log": "Không còn node con nào để xét."
            }
            return
        ...
        if best_h < current_h:
            ...
            yield {
                ...
                "frontier": [],
                "explored": explored,
                "log": f"Chọn node tốt nhất {current_node.state[0]}, h giảm từ {current_h} xuống {best_h}"
            }
        else:
            yield {
                "current": current_node,
                ...
                "done": True,
                "path": reconstruct_path(current_node),
                "log": "Dừng vì không có node con nào tốt hơn. Bị kẹt tại cực tiểu cục bộ."
            }
            return
